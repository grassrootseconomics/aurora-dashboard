            ...
            <div className="dashboard__cards dashboard__cards--main">
              ...
              </button>
              {
                userRole == UserRole.project ? 
                  <>
                    <button className={"dashboard__download-button"} onClick={handleOpenHarvestingModal}>{t("buttons.set_next_harvesting")}</button>
                    <HarvestingModal open={openHarvestingModal} onClose={handleCloseHarvestingModal}></HarvestingModal>
                  </>
                  : ""
              }
            </div>
          ...
          {userRole == UserRole.buyer ? (
            <div className="dashboard__cards home_cards">
              ...
              <CardFive
                ...
                number={
                  selectedDepartment ?
                    convertToMonthYearDate(departments[selectedDepartment - 1]?.nextHarvest, i18n.language) :
                    (new Date (Math.min(...departments.map(dep => new Date(dep.nextHarvest).getTime())))).toLocaleString(i18n.language, { month: "long", year: "numeric" })
                  }
                text={
                  selectedDepartment
