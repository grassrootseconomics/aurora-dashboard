...
const HarvestingModal: FC<HarvestingModalProps> = ({
  open,
  onClose,
}) => {
  ...
  const handleConfirm = (data: {idDepartment: number, nextHarvest: string}) => {
  ...
  const handleIdDepartmentChange = (selectedOption: any) => {
    ...
    setValue('nextHarvest', convertToSimpleDate(departments?.find(d => d.id == selectedOption)?.nextHarvest ?? ""));
  };
