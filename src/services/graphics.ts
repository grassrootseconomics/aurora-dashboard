...
export const getProductionPerAssociationsGraph = (data: any) => {
  ...
  return productionCocoa.map((value, index) => {
    return new Dataset(value.name, value.values, graphColors[index % 5]);
  });
};
...
export const getPriceOfOrganicCocoaGraph = (data: any) => {
  return [
    new Dataset(
      t('home.usd_price'),
      data?.report.priceOfOrganicCocoa.map(
        (value: any) => value.organicSoldPrice
      ),
      graphColors[2]
    ),
  ];
