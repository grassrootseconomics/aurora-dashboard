...
import { mapToProducerActivity, ProducerActivity } from "@/util/models/Producer/ProducerActivity";

export const getProducersStatistics = async (association?: string): Promise<any> => {
...
export const getBasicProducers = async (association?: string): Promise<BasicProducer[]> => {
  ...
  return response.data.data.searchProducersResult.data.map((p: any) => 
    new BasicProducer(p.code, 
      p.firstName,
      p.lastName,
      p.village,
...
export const getProducerActivity = async (code: number): Promise<ProducerActivity> => {
...
export const getPulpByCode = async (producerCode: string, pulpId: number): Promise<Pulp> => {
...
export const updateProducer = async (code: string, producer: any): Promise<any> => {
