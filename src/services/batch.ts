...
export const getBatchesWithoutAuth = async (): Promise<any> => {
...
export const getAvailableBatchesForBuyer = async (department?: string): Promise<any> => {
    const response: AxiosResponseData<any> = await api.get(
      `/v1/batch`, {
        params: {
    ...
    return batchesArray.map(b => new BasicAvailableBatch(b.code, 
        b.storage.netWeight, 
        b.fermentationPhase.cocoaType,
        b.fermentationPhase.startDate,
        b.fermentationPhase.humidity,
...
export const getAvailableBatchesStatisticsForBuyer = async (department?: string): Promise<BatchStatistics> => {
  const response: AxiosResponseData<any> = await api.get(
    `/v1/batch`, {
      params: {
        department: department,
...
export const getPulp = async (code: string): Promise<BuyerBatchInfo> => {
  const response: AxiosResponseData<any> = await api.get(
      `/v1/batch/${code}`
  );
  const pulpsUsed = response.data.data.batch.pulpsUsed.find();
  return mapToBuyerBatchInfo(pulpsUsed);
}
