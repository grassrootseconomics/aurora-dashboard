...
import { BuyerBatchInfo, mapToBuyerBatchInfo } from "@/util/models/Batch/BuyerBatchInfo";
import { DailyReport, Flip } from "@/util/models/Batch/Fermentation";
import { EmailSample } from "@/util/models/EmailSample";

export const getBatchesWithoutAuth = async (department? : string): Promise<any> => {
...
export const getBatchByCodeForBuyers = async (code: string): Promise<BuyerBatchInfo> => {
    const response: AxiosResponseData<any> = await api.get(
        `/v1/batch/${code}`
    );
    const batch = response.data.data.batch;
...
export const getAvailableBatches = async (association?: string): Promise<any> => {
    const response: AxiosResponseData<any> = await authenticatedApi.get(
      `/v1/batch/available`, {
        params: {
    ...
    const basicBatches = batchesArray.map(b => new BasicAvailableBatch(b.code, 
        b.storage.netWeight, 
...
export const getSoldBatches = async (association?: string): Promise<any> => {
    ...
    const basicBatches = batchesArray.map(b => new BasicSoldBatch(b.code, 
        b.sale.buyer,
        b.sale.destination,
        b.sale.pricePerKg,
        b.sale.totalValue,
...
export const updateFermentationPhase = async (salesStats: any[]): Promise<any> => {
  const response: AxiosResponseData<any> = await authenticatedApi.patch(
...
export const addDayReport = async (fermentationId: number, dayReport: DailyReport): Promise<any> => {
...
export const addFlip = async (fermentationId: number, flip: Flip): Promise<any> => {
  const response: AxiosResponseData<any> = await authenticatedApi.post(
...
export const editDayReport = async (fermentationId: number, dayReport: DailyReport, index: number): Promise<any> => {
  const response: AxiosResponseData<any> = await authenticatedApi.patch(
...
export const editFlip = async (fermentationId: number, flip: Flip, index: number): Promise<any> => {
  const response: AxiosResponseData<any> = await authenticatedApi.patch(
...
export const deleteDayReport = async (fermentationId: number, index: number): Promise<any> => {
  const response: AxiosResponseData<any> = await authenticatedApi.delete(
    `/v1/batch/${fermentationId}/fermentation/reports/${index}`);
  return response;
};

export const deleteFlip = async (fermentationId: number, index: number): Promise<any> => {
  const response: AxiosResponseData<any> = await authenticatedApi.delete(
    `/v1/batch/${fermentationId}/fermentation/flips/${index}`);
  return response;
};

export const sendSampleEmail = async (codeBatch: string, emailFields: EmailSample): Promise<any> => {
